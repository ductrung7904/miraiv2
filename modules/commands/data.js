		...
		case "user":
			{
				...
				return request(encodeURI(`https://graph.facebook.com/${idd}/picture?width=512&height=512&access_token=4113544655391303|DSNzTsbi-lyjh9a_y3VZuursQJ0`)).pipe(fs.createWriteStream(__dirname + '/cache/2.png')).on('close', () => callback());
		
			}	
			break;

		default:
			return api.sendMessage("Syntax error, use : data user/box [ID]", event.threadID);
			break;
	}
}