...
module.exports.handleReply = async function({ api, args, Users, handleReply, event, Threads }) {
    ...
    switch (handleReply.type) {
        case 'unbanthread':
            {
                const data = (await Threads.getData(myString)).data || {};
                data.banned = 0;
                await Threads.setData(myString, { data });
                global.data.threadBanned.delete(parseInt(uid), 1);
                api.sendMessage(`[${myString}] Đã unban thành công!`, threadID);
                break;
            }
			
		case 'unbanuser':
		{
			const data = (await Users.getData(myString)).data || {};
			data.banned = 0;
			await Users.setData(myString);
